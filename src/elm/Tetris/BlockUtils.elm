...
stompSelection selection blocks =
    ...
    let
        column =
            col selection.location

        start =
            row selection.location
        ...
        Matrix.map
            (\block ->
                if inColumn column start end block then
                    let
                        maybeAboveBlock =
                            Matrix.get (loc ((row block.location) - 1) column) blocks
                    in
                        case maybeAboveBlock of
                            Nothing ->
                                emptyBlock (loc 0 column)

                            Just aboveBlock ->
                                { aboveBlock | location = block.location }
                else
                    block
            )
            blocks
